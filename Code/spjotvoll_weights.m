function [w,c] = spjotvoll_weights(mu,q)
...
if (gamma >= pfer)
    [w_neg,c] = spjotvoll_weights_finder(mu(neg_ind),q,J);
    w(neg_ind) = w_neg;
else
    %case 2: the mass at negative means is small enough
    %then all weights are defined by the formula Phi(mu/2 + c/mu)
    if (gamma+n0 <= pfer)
        [w_neg,c] = spjotvoll_weights_finder(mu(neg_ind),J - n0/q);
        w(neg_ind) = w_neg;
        w(~neg_ind) = 1/q;
    else
        %case 3: the mass at negative means is intermediate
        w(neg_ind) = normcdf(mu(mu<0)./2)/q;
        w(~neg_ind) = (pfer-gamma)/(q*n0);
        c = NaN;
    end
end